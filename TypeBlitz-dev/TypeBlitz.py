    ...
    def scheme_first():
        ...
        def app_updator():
            ...
            def updator_engine():
                ...
                def is_uptodated():
                    ...
                    if latest_version > local_version:
                        ...
                        def ask_continue():
                            ...
                            if msg_box == "yes":
                                exe_url = "https://sarwanboi.github.io/TypeBlitz/production/latest/TypeBlitz.exe"
                                ...
                                try:
                                    def run_download():
                                        ...
                                        def install():
                                            droot.destroy()
                                            wordtk.destroy()
                                            os.startfile("TypeBlitz - Setup.exe")
                                        ...
                                        def download(url):
                                            filename = "TypeBlitz - Setup.exe"
                                            block_size = 1024  # 1 Kibibyte
                                            headers = {
                                                "User-Agent": "Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/89.0.4389.82 Safari/537.36"
                                            }

                                            labeln.configure(text="Downloading... 0%", foreground="#1E90FF", font=("Calibri (Body)", 12, "bold"))
                                            ...
                                            with requests.get(url, headers=headers, stream=True) as response:
                                                ...
                                                with open(filename, "wb") as f:
                                                    for data in response.iter_content(block_size):
                                                        ...
                                                        labeln.configure(text=f"Downloading... {percentage:.2f}%", foreground="#1E90FF", font=("Calibri (Body)", 12, "bold"))
                                                        f.write(data)
                ...
                version_url = "https://sarwanboi.github.io/TypeBlitz/production/version.txt"
        ...
        def para_page():
            ...
            def para_history_panel():
                ...
                root.iconbitmap("TypeBlitz_Icon.ico")
                ctypes.windll.shcore.SetProcessDpiAwareness(1)
            ...
            def count(stop_flag): 
                while stop_flag[0]:
                    global wrongwords, timing, totaltime, elapsedtimeinminutes, textarea  
                    ...
                    try:
                        ...
                        if not table_exists:
                            conn.execute("""CREATE TABLE typing_stats (ACCURACY REAL, WPM REAL, GROSS_WPM REAL, TOTAL_WORDS INTEGER, WRONG_WORDS INTEGER, TIMESTAMP TIMESTAMP DEFAULT CURRENT_TIMESTAMP)""")
                        conn.execute("INSERT INTO typing_stats (ACCURACY, WPM, GROSS_WPM, TOTAL_WORDS, WRONG_WORDS) VALUES (?, ?, ?, ?, ?)",(accuracy, wpm, gross_wpm, totalwords, wrongwords))
            ...
            def start():
                ...
                start_threads(stop_flag, textarea) 

            def start_threads(stop_flag, textarea):
                try:
            ...
            image_files = ["paramain.png","parafooter.png", "paraback.png", "parahistory.png","pararesult.png", 
                           "paramid.png","paragraphmain.png","resultframe.png","textarea_para.png", "button_start.png", "button_reset.png"]
            ...
            paradef = Frame(wordtk, bg=bgcolor, width=1280, height=700, background=parabg)
            paradef.pack()
            
            para_main = Frame(wordtk, width=1200, height=620, background=parabg)
        ...
        def home_page():
            def exitway():
                def finalexit():
                    nonlocal images
                    try:
                        print("\033[31m// Closing Application!!\033[0m")
                        try:
                            for image in images.values():
                                if image:
                                    ...
                                    del image
                            images = {}
                            print(" -->> \033[32mCache Successfully Cleared: \033[33mHOME PAGE")
                            wordtk.destroy()
                            print("\033[31m// Exit Execution Succesfully Done.\033[0m")
                        except Exception as e:
            ...
            wordbtn = Button(option_in_a, image=images["button_word.png"], background="#FFF", activebackground="#FFF", border=0, command=open_word_page)
            wordbtn.place(x=36, y=140)
            parabtn = Button(option_in_a, image=images["button_paragraph.png"], background="#FFF", activebackground="#FFF", border=0, command=open_para_page)
            ...
            wordtk.bind("<B1-Motion>", move_app)
            wordtk.protocol("WM_DELETE_WINDOW", exitway)
            wordtk.mainloop()
            # ***************************************************************************************************************************************************  #
